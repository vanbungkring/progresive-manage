...
var campaignController = {
  ...
  add: function(req, res) {
    if (req.method === 'POST') {
      ...
      construct.writer = req.user._id ? req.user._id : 0;
      construct.publishedStatus = req.body.publishedStatus ? 1 : 0;
      construct.featured = req.body.featured ? true : false;
      post.title = req.body.title;
      post.images = req.body.images;
      post.slug = global.library.SLUGG(req.body.title);
      post.tag = req.body.tags ? req.body.tags.split(',') : [];
      ...
      campaignVM.createCampaign(construct, function(result) {
        ...
        var msg = (result.status === global.status.STATUS_FAILED) ? 'Gagal menambahkan campaign' : 'Berhasil menambahkan campaign ' + post.title;
  ...
  edit: function(req, res) {
    ...
    if (req.method === 'GET') {
      ...
      //construct.merchant = req.body.merchant ? req.body.merchant : req.user.merchant._id;
      construct.campaignCategory = req.body.campaignCategory;
      construct.bank = req.body.bank ? req.body.bank : [];
      ...
      model.campaign.update({
        ...
        var msg = err ? 'Gagal merubah campaign' : 'Berhasil merubah campaign ' + post.title;
        global.library.FLASH(req, state, globalStatus, msg);
        res.redirect(PREFIX_ROUTE_BACK_OFFICE + '/campaign');
      });
    }
      ...
      campaignVM.findCampaign(constructParams, function(result) {
        if (result.data) {
          model.campaign.remove({
            ...
            var globalStatus = err ? global.status.STATUS_FAILED : global.status.STATUS_SUCCESS;
            var msg = err ? 'Gagal menghapus campaign' + result.data.postMeta.title : 'Berhasil menghapus campaign ' + result.data.postMeta.title;
            global.library.FLASH(req, state, globalStatus, msg);
            res.redirect(PREFIX_ROUTE_BACK_OFFICE + '/campaign');
          });
        }
