...
var categoriesController = {
  ...
  add: function(req, res) {
    if (req.method == 'POST') {
      ...
      construct.status = req.body.status ? 1 : 0;
      ...
      categoriesVM.createCategory(construct, function(result) {
        ...
        var msg = (result.status === global.status.STATUS_FAILED) ? 'Gagal menambahkan kategori' : 'Berhasil menambahkan kategori ' + req.body.name;
  ...
  edit: function(req, res) {
    if (req.method == 'POST') {
      ...
      model.categories.update({
        ...
        var msg = err ? 'Gagal merubah kategori' : 'Berhasil merubah kategori ' + req.body.name;
        global.library.FLASH(req, state, globalStatus, msg);
        res.redirect(PREFIX_ROUTE_BACK_OFFICE + '/categories');
      });
    } else {
      ...
      categoriesVM.findCategory(constructParams, function(result) {
        if (result.data) {
          model.categories.remove({
            ...
            var globalStatus = err ? global.status.STATUS_FAILED : global.status.STATUS_SUCCESS;
            var msg = err ? 'Gagal menghapus kategori' + result.data.name : 'Berhasil menghapus kategori ' + result.data.name;
            global.library.FLASH(req, state, globalStatus, msg);
            res.redirect(PREFIX_ROUTE_BACK_OFFICE + '/categories');
          });
        }
